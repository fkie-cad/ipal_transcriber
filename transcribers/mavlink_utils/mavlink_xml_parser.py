...
def decode_hex(variable, field_type):
    ...
    if field_type == "char":
        ...
        # TODO possible to "pretty print" strings instead of char arrays
...
def parse_XML(file):
    ...
    messages_ordered = order_messages(messages)
...
def parse_into_rules(messages):
    ...
    for m_id, m in messages.items():
        ...
        last_index = 0
        for field in fields:
            rule_name = f"{field['name']}({field['enum']})" if 'enum' in field else field['name']
            ...
            if '[' not in field_type:
                ...
                # Type is an array
                var_field_len = type_to_hexlen(field_type.split('[')[0])  # get field type before [X]
                field_len = (int(field_type[field_type.index('[') + 1:-1]) * var_field_len)  # X*type_length
            ...
            rule = {'rule_type': rule_type, 'rule_method': rule_method, 'rule_name': rule_name}
            rules.append(rule)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file", default="common.xml")
    parser.add_argument("--output", default="rules_mavlink.py")
    args = parser.parse_args()
    file = args.file
    output_file = args.output

    messages_ordered, enums = parse_XML(file)
    rules = parse_into_rules(messages_ordered, enums)
